if ('__TAURI__' in window) {
  var __TAURI_PLUGIN_DEEP_LINK__ = (function (e) {
    ...
    async function i(e, t, i) {
      const _ = { kind: 'Any' };
      return r('plugin:event|listen', { event: e, target: _, handler: n(t) }).then(
        (n) => async () =>
          (async function (e, n) {
            window.__TAURI_EVENT_PLUGIN_INTERNALS__.unregisterListener(e, n),
              await r('plugin:event|unlisten', { event: e, eventId: n });
          })(e, n),
      );
    }
    return (
      ...
      (e.getCurrent = async function () {
        return await r('plugin:deep-link|get_current');
      }),
      (e.isRegistered = async function (e) {
        return await r('plugin:deep-link|is_registered', { protocol: e });
      }),
      (e.onOpenUrl = async function (e) {
        return await i('deep-link://new-url', (n) => {
          e(n.payload);
        });
      }),
  ...
  Object.defineProperty(window.__TAURI__, 'deepLink', { value: __TAURI_PLUGIN_DEEP_LINK__ });
}
