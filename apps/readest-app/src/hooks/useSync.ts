  ...
  const pullChanges = async (
    ...
    try {
      ...
      if (err instanceof Error) {
        // TODO: When talebook server sync is connected, handle auth errors here.
        // Original: if (err.message.includes('Not authenticated') && latest.keepLogin) {
        //   latest.keepLogin = false; setSettings(latest); navigateToLogin(router); }
