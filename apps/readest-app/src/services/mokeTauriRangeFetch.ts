...
export type MokeTauriInvoke = <T>(command: string, args?: Record<string, unknown>) => Promise<T>;
...
/**
 * Send an already validated HEAD or single-range GET directly through
 * tauri-plugin-http. ArkWeb can remove `Range` when the public plugin wrapper
 * first constructs a browser `Request`; bypassing only that normalization
 * keeps the native URL ACL and shared authenticated cookie jar intact.
 */
export function createMokeTauriRangeFetch(invoke: MokeTauriInvoke = tauriInvoke) {
  return async (url: string, init: RequestInit = {}): Promise<Response> => {
    const method = (init.method || 'GET').toUpperCase();
    if (method !== 'HEAD' && method !== 'GET') throw new Error('online.request_invalid');
    ...
    const abort = () => {
      ...
      try {
        ...
        // The stream may already be closed while the WebView is being replaced.
      }
    };
...
export const mokeTauriRangeFetch = createMokeTauriRangeFetch();
